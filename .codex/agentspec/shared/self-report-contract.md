...
> **Phase C note — judgment fields:** `all_claims_cited`, `all_cited_ids_in_input`, `groundedness_pass`, and `coverage_ledger_consistent` are agent self-assessments. The skill **must recompute** all four deterministic gates — it must **not gate blindly on the agent's booleans**. The agent's `citations` list is the authoritative output; the booleans are its own consistency check, not a ground truth the skill should skip verifying. `all_cited_ids_in_input` is recomputed by joining `citations[*].chunk_id` against the skill-retained `ranked_chunks[*].point_id`; `groundedness_pass` is re-verified using the same lexical-overlap rule (cited chunk text shares ≥1 key term with the claim); `all_claims_cited` is re-verified by checking factual answer sentences against citation claim phrases; `coverage_ledger_consistent` is re-verified from `sub_query_ledger`, `coverage_verdict`, and `qualification`.
